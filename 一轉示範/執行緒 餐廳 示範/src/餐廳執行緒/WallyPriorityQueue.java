 ...
 * @author Wally
 */
public class WallyPriorityQueue<T> implements Iterable<T> {
	...
	public	WallyPriorityQueue(Comparator<T> c){
		myComparator = c ;
	}
	...
	private void normalSort() {
		...
		for (int i = 0 ; i < arrayTElementAmount ; i++ ) {
			...
			for	(int j = 0 ; j < arrayTElementAmount ; j++ ) {
				...
				if 	( myComparator.compare((T)arrayT[i], (T)arrayT[j]) > 0 ) {
					compareResultNumber++ ;
				}
			}
	...
	public boolean isEmpty(){
		return popIndex == arrayTElementAmount;
	}
