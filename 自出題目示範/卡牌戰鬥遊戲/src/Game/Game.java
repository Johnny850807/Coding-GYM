...
public class Game {
	...
	public void start(){
		definePlayerTypePhase();
		inputNamePhase();
		gameStartingPhase();
	}

	private void definePlayerTypePhase() {
	...
	private void inputNamePhase() {
		player1.setName(player1.createName());
		player2.setName(player2.createName());
	}
	
	private void gameStartingPhase(){
		for ( int i = 0 ; i < 7 ; i ++ )
			oneRoundPhase();
		onGameResultPhase();
	}

	private void oneRoundPhase() {
	...
	private void onGameResultPhase() {
		Player winner;
